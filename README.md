...
模拟 join 的 0–65535 范围在各平台内部映射到对应业务单位（亮度 0–255、音量 0–1、温度 ×10 等）。窗帘位置例外：主控直接用 0–100 模拟量表示百分比（0=全关，100=全开），不走 0–65535 满量程。
...
**⚠️ 前提与坑（排查必看）：**
- **同步是否正确，取决于主控程序是否为该 join 配了"状态回传"**。只接收命令、不回传状态的 command-only join，HA **永远无法**得知其真实状态——协议没有查询能力，只能在 SIMPL 侧补回传。本仓库两个真实例子：空调电源改读**会回传**的 `on_join` 电平；只开关灯改读 `on_join`/`off_join` 的回传电平。
...
- `pos_join`：可选，模拟 join，**0–100** 直读直写（0=全关，100=全开），非 XSIG 0–65535 满量程。
  - 主控**有**回传位置时：卡片显示可拖的百分比滑块（`SET_POSITION`），用真实位置。
  - 配了 `pos_join` 但主控**没回传**位置时（快思聪侧常见缺口）：自动降级为「命令推断」——点开=100%、点关=0%、停=保持当前，只显示开/关/停按钮（不给滑块，避免「能精确定位」的错觉），并用 `RestoreEntity` 跨重启恢复。**一旦主控开始回传位置，自动升级回真实位置 + 滑块，无需改配置。**
